...
- The client-engine program (`architecture/`) keeps its items in `BACKLOG-ENGINE.md`.
  Ids are unique across both files: a new item takes the next free number of its letter
  in either.
...
### T-8 · `script-embedding.spec.ts` sorts through a header button that is gone · `done` (2026-09-25, feat/eval-tables) · *2026-09-24*
"script column: ref snippet computes values + error cell + sorts" waited at `:205` for a
`Sort by` button in the table header; sorting moved into the definition and
`Table/ColumnSortDialog.svelte` with `edc60f2` (2026-09-15). Fixed: the test now drives
