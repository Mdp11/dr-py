...
describe('the artifact follower and rule sets', () => {
	...
	it('a rule set the own commit created goes into the committed layer at once, not the overlay', async () => {
		const over = await swept();
		const f = follow(over.sync);
		f.follower.load();
		await f.follower.settled();
		...
		expect(f.follower.hasOverlay()).toBe(true);
		expect(f.follower.kindOf('r9')).toBe(RULES_KIND);
		expect(await ruleIssuesOf(over)).toEqual(ruleIssues('de-only', NOT_DE, 'on_server'));

		refresh.resolve();
		await f.follower.settled();
		expect(f.fetches.at(-1)).toEqual(['r9']);
		expect(f.posted.at(-1)).toEqual({
			method: 'putArtifacts',
			changed: [made],
			deletedIds: [],
	...
	it('a rule set the own commit updated goes into the committed layer at once, its name kept', async () => {
		const over = await swept();
		const f = follow(over.sync);
		f.committed.set('r1', ruleSet('r1', 'Rules', A, parsed(DE_ONLY)));
		f.follower.load();
		...
		f.committed.set('r1', ruleSet('r1', 'Rules', B, parsed(DE_OR_FR), 2));
		const refresh = f.gate();
		f.setStaged([]);
		f.follower.stagedChanged();
		f.follower.onCommit({ idMap: {}, changed: [header(f.committed.get('r1')!)], deletedIds: [] });
		expect(f.posted.at(-1)).toEqual({
			method: 'putArtifacts',
			changed: [
				{
					...
					name: 'Rules',
					artifact_rev: 2,
					payload: rulesPayload(B),
					rules: document(parsed(DE_OR_FR))
				}
			],
			deletedIds: [],
			staged: []
		});
		expect(await ruleIssuesOf(over)).toEqual(ruleIssues('de-or-fr', NOT_DE_OR_FR, 'on_server'));

		refresh.resolve();
		await f.follower.settled();
		expect(f.fetches.at(-1)).toEqual(['r1']);
		expect(await ruleIssuesOf(over)).toEqual(ruleIssues('de-or-fr', NOT_DE_OR_FR, 'on_server'));
		expect(f.parsesAsked).toEqual([B]);
	});
