...
- A `validation_rules` artifact carries `document` (the server-parsed rule set) or
  `parse_errors`; the engine never reads its YAML.
- `artifacts_version` moves on every change to either layer; it stamps the table order cache
  (§4) and the rules compile (§3).
...
- A call that changes the effective rule set (a rules artifact staged, committed or deleted)
  ...
  background scan (§3); plan 3 decides whether that moves these methods into the lane.

### Server

- `GET /artifacts/payloads` (read-only, viewer-callable): every artifact of the project with
  its payload and `artifact_rev`; for `validation_rules`, `document` or `parse_errors` beside
  the stored YAML.
- `POST /rules/parse {yaml}` → `{ok, document | errors}` (§3).
...
- `artifact-edits.svelte.ts` stays the owner; one subscriber pushes `setStagedArtifacts` on
  ...
  it has been, it is pushed as `parse_pending` and the rule set keeps its previous compile.
...
- Cached per `(rev, staged_version)` — `artifacts_version` is not added until plan 3 (rules are
  the first thing the probe validates that reads an artifact); requested lazily by the issues
  read and the preview, never per keystroke.
...
- **Server:** `POST /rules/parse {yaml}` → `{ok, document | errors}` — lint's parser and caps
  (64 KiB, 200 rules, depth 8, no aliases, unique names), `document` = the normalized `RuleSet`
  (`model_dump(mode="json")`); reads no model; the lint route's 403 for viewers.
  `GET /artifacts/payloads` carries the same `document`.
...
- **A rule set change** recompiles inside the artifact transition; revalidating
  `applies_population(old ∪ new)` is a background scan, `rules_status.applying` true until it
  ends.
