...
Deterministic builder (no RNG seed needed) that produces ~1000 elements and
~340 relationships. Designed to pass every validator in
`data_rover.core.validation` against the smart-city metamodel.
...
def _eid() -> str:
    return f"e_{next(_eid_seq):06d}"


def _rid() -> str:
    return f"r_{next(_rid_seq):06d}"
...
def mk(type_name: str, **props: Any) -> Element:
    e = Element(id=_eid(), type_name=type_name, properties=props)
    elements.append(e)
    return e
...
def cycle(seq: Iterable[Any]) -> "itertools.cycle[Any]":
        ...
        mk(
            ...
            **{
                **base_owned(i, "PERF"),
                ...
                "criticality": CRITICALITIES[i % 4],
                "metric": "latency_p99",
                "target_value": float(50 + i * 5),
                "unit": "ms",
                "latency_p99_ms": float(50 + i * 5),
...
# 8 AZs distributed across regions (containment satisfied below).
zones: list[Element] = []
for i in range(1, 9):
    zones.append(
        mk(
        ...
        mk(
            ...
            **{
                **base_owned(i, "Proj"),
                ...
                "start_date": _date(i * 7),
                "end_date": _date(i * 7 + 365),
...
for i in range(8):
    rel("Refines", functional_reqs[i], functional_reqs[(i + 1) % len(functional_reqs)])
for i in range(6):
...
contained_components: set[str] = set()
for i, comp in enumerate(all_components):
    if i >= 180:
        break  # leave a few uncontained
    sys = all_systems[i % len(all_systems)]
    rel("SystemContainsComponent", sys, comp)
    contained_components.add(comp.id)
...
already_served: set[str] = set()
for i, dev in enumerate(iot_devices[:30]):
    if dev.id in already_served:
        continue
    rel("GatewayServes", gateways[i % len(gateways)], dev)
    already_served.add(dev.id)
...
# Give half of the endpoints a request schema, and some also a response schema.
for i, ep in enumerate(api_endpoints[:40]):
    rel(
        "DefinedBy",
        ep,
...
# GovernedBySchema: DataEntity -> DataSchema (target_multiplicity 0..1)
governed: set[str] = set()
for i in range(15):
    de = data_entities[i]
    if de.id in governed:
        continue
    rel("GovernedBySchema", de, data_schemas[i % len(data_schemas)])
    governed.add(de.id)
...
# AssignedTo: Risk -> Person (target_multiplicity 0..1)
assigned: set[str] = set()
for i in range(10):
    r = risks[i % len(risks)]
    if r.id in assigned:
        continue
    rel("AssignedTo", r, persons[i % len(persons)])
    assigned.add(r.id)
...
documentable = software_systems + all_components[:20] + processes[:5] + data_entities[:5]
for i in range(15):
