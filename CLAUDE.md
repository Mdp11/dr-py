...
- **Settings (`api/settings.py`)** — `snippet_runner` (`"wasm"`|`"trusted"`, default `"wasm"`), `snippet_guest_wasm_path`/`snippet_guest_lib_path` (vendor binary/stdlib, unfetched by default), `snippet_pool_size` (default 2), `snippet_concurrency`/`snippet_per_user_concurrency` (4/1), and the `RunLimits` mirrors `snippet_wall_timeout_s`/`snippet_memory_bytes`/`snippet_stdout_bytes`/`snippet_result_repr_bytes`/`snippet_max_ops`/`snippet_max_op_bytes`/`snippet_page_limit` — all `DATA_ROVER_SNIPPET_*` env vars (standard `DATA_ROVER_` prefix).
- **Guest binary** — `spikes/code_exec/vendor/python.wasm` + stdlib are NOT committed; fetch via `bash spikes/code_exec/fetch_python_wasi.sh` (idempotent). Without it, `main._boot_script_runner` logs a warning and leaves the runner `None` (routes 503) rather than failing startup. The `integration`-marked test (`tests/api/test_snippets_wasm.py`) exercises the real sandbox end-to-end and needs the binary fetched first.
...
- **Embedded evaluation (M2/M3 — `ScriptColumn`/`ScriptStep`)** — table columns and navigation steps evaluate a snippet's `value(elements)`/`step(el)` against the live model via `ScriptRunner.open_session`: one warm guest instance per evaluation (keyed by code), execs the facade + module once, then serves repeated entry-point calls instead of a fresh boot per cell. `core/script/embed.py`'s `ScriptEvalContext` is the per-request state threaded through both table and navigation evaluation — memoizes calls by `(code, entry, element_ids)`, collects `.warnings`, and enforces a shared `ScriptBudget` (`snippet_eval_budget_s` setting). The stance is **degraded, not failed**: a missing runner, a full concurrency slot, or a snippet error never 5xxs a table/nav route — it renders error cells (`ScriptColumn`) or pruned-with-warning chains (`ScriptStep`), and the route stays 200. Embedded work draws one **global** slot (no per-user cap) per evaluate/export request from the same shared guard console runs use (`api/snippet_concurrency.py`). `routes/tables.py`'s `TableOrderCache` has a cache-poisoning guard: a row order is only cached when nothing in the request errored (`script_ctx.errored`) and the model didn't move mid-build, since neither the fingerprint nor `rev` would change on retry to invalidate a bad entry. Full mechanics (wire protocol, memo soundness caveat, per-call deadline) are in `core/script/README.md`'s "Evaluation sessions (M2/M3)" section.
