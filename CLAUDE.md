...
- **`Commit` row additions** (Alembic 0003) — `message TEXT`, `validation_error_count INTEGER`, `issues JSONB` on the existing `commits` table; fed by `_persist_commit` via the extended `_commit_id`/`_message`/`_validation_error_count`/`_issues` kwargs.
