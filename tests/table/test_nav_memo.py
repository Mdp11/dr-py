...
def test_sort_by_step_ref_navigates_once_per_root(monkeypatch):
    ...
    calls.clear()
    ordered = order_rows(mm, model, defn, built.keys, [SortSpec(column=1, direction="asc")], base_slots=built.base_slots)
...
def test_sort_by_step_ref_over_value_terminal_navigates_once_per_root(monkeypatch):
    ...
    built = build_rows_ex(mm, model, defn)
    calls = _count_evaluate(monkeypatch)
    order_rows(mm, model, defn, built.keys, [SortSpec(column=1, direction="desc")], base_slots=built.base_slots)
...
def test_script_navigation_bypasses_memo(monkeypatch):
    ...
    calls = _count_evaluate(monkeypatch)
    order_rows(mm, model, defn, built.keys, [SortSpec(column=1, direction="asc")], base_slots=built.base_slots)
...
def test_cells_window_navigates_once_per_root(monkeypatch):
    ...
    built = build_rows_ex(mm, model, defn)
    calls = _count_evaluate(monkeypatch)
    cells = evaluate_cells(mm, model, defn, built.keys, base_slots=built.base_slots)  # build order: roots contiguous
    assert len(calls) == N_ROOTS
...
def test_cells_pass_never_reuses_an_earlier_pass_result():
    ...
    cells = evaluate_cells(mm, model, defn, ordered, base_slots=built.base_slots)
