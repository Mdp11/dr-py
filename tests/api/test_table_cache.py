...
def test_put_get_roundtrip():
    ...
    c.put(fp, 5, (("x",), ("y",)), False, 2, 3)
    assert c.get(fp, 5) == ((("x",), ("y",)), False, 2, 3)


def test_truncated_flag_roundtrips():
    c = TableOrderCache()
    fp = table_fingerprint('{"a":1}')
    c.put(fp, 5, (("x",),), True, 1, 1)
    assert c.get(fp, 5) == ((("x",),), True, 1, 1)


def test_stale_rev_is_a_miss():
    c = TableOrderCache()
    fp = table_fingerprint('{"a":1}')
    c.put(fp, 5, (("x",),), False, 1, 1)
...
def test_lru_evicts_beyond_cap():
    ...
    for i in range(3):
        c.put(table_fingerprint(f'{{"a":{i}}}'), 1, ((str(i),),), False, 1, 1)
    # oldest (i=0) evicted
...
def test_session_touch_model_clears_cache():
    ...
    s = Session()
    s.table_order_cache.put(table_fingerprint("{}"), 0, (("x",),), False, 1, 1)
    s.touch_model()
