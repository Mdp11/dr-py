...
class JsonColumnOptions(BaseModel):
    ...
    group: bool = False
    #: Render an array cell as ONE value (`key: value`): zero -> `null`, one
    #: -> that item (an element per `value`, a value as is), more -> the
    #: export fails (422) naming the column. Strict rather than tolerant
    #: because the shape is a contract a consumer branches on. Collapses
    #: BOTH array kinds a navigation can yield per row (elements, or the
    #: values a scalar property step reaches); ignored, like `value`, on a
    #: cell that is never an array.
    single: bool = False
