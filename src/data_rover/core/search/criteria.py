...
def name_prop(props: dict[str, object]) -> str | None:
...
def match_element(model: Model, e: Element, c: Criterion) -> bool:
    ...
    if isinstance(c, NameIdCriterion):
        return _match_name_id(name_prop(e.properties) or "", e.id, c)
...
def match_relationship(model: Model, r: Relationship, c: Criterion) -> bool:
    ...
    if isinstance(c, NameIdCriterion):
        return _match_name_id(name_prop(r.properties) or "", r.id, c)
