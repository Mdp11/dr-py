...
def run_snippet(
    ...
    _, artifact_ops, view_ops, metamodel_ops = split_ops(validated_ops)
    if artifact_ops or view_ops or metamodel_ops:
        ...
        # artifact, view AND metamodel op families — so without this, a
        # snippet could propose `update_artifact` rewriting ANOTHER snippet's
        # code, a view op reshaping the folder tree, or a metamodel rebind,
        # and have it staged for an editor who believes they are approving
        # the data edits they asked for. `create_artifact` needs no lease at
        # all, so nothing downstream would stop it landing, and
        # /snippets/run is a read-only POST: the proposing author can be a
        # VIEWER while the approver is an editor — a confused deputy. The
        # guest facade has no artifact, view OR metamodel surface whatsoever,
        # so any of these ops here is by definition not legitimate; refused
        # exactly like the malformed batch above (server-side fault, never
        # surfaced to the client for staging).
        logger.error(
            "snippet run %s emitted artifact/view/metamodel ops (not a guest "
            "capability): %r",
            payload.run_id,
            res.ops,
        )
